...
;; Copyright (C) 2001-2003 Bill St. Clair
...
;; The name of the table-of-contents file containing the name and
;; headers of files in current directory
(defconst *weblog-toc-file* "top.txt")

;; The table of contents for current directory
(defvar *weblog-toc* nil)
...
;; Bound the the file being generated
...
(defconst *weblog-bracket-char* (string-to-char "{"))
  ...
  (dolist (file (directory-files dir t))
    (when (file-directory-p file)
      (let ((name (file-name-nondirectory file)))
        (unless (or (equal name ".") (equal name "..") (equal name ".svn"))
          (weblog-map-all-files file pred f pattern))))))
  ...
  (let ((file-name (buffer-file-name))
    ...
    (weblog-with-init-params file-name
      (if (null file-name)
        ...
        (if (eq file-name *weblog-toc-file*)
            (setq *weblog-toc* ())
          (weblog-parse-toc-file (file-name-directory file-name)))
...
(defun weblog-update-directory ()
  "Regenerate html for every \".txt\" file
in the current directory"
  ...
  (weblog-map-all-files
   (file-name-directory file)
   '(lambda (file)
      (weblog-while-visiting-file buf file (weblog-save)))))
    ...
    (let* ((time (decode-time (current-time)))
      ...
      (if (and (eq year cur-year)
	       (equal (weblog-first-text-file-this-month)
		      (weblog-latest-text-file)))
	  (progn
	    (setq month (1- cur-month))
	    (when (eq 0 month)
	      (setq month 12)
	      (decf year)))
...
Upload it to the FTP server."
  ...
  (weblog-with-init-params (buffer-file-name)
    ...
    (weblog-while-visiting-weblog-file rss-buf "rss.xml"
      (let* ((template (weblog-file-contents "rss-template.xml"))
...
(defun weblog-macro-tableOfContents ()
  *weblog-toc*)
...
(defun weblog-file-mdy (file-name)
  ...
  (ignore-errors
    (let* ((name (file-name-sans-extension
           ...
           (yy (car (read-from-string (substring name 0 2))))
           (mm (car (read-from-string (substring name 2 4))))
           (dd (car (read-from-string (substring name 4 6)))))
      (when (and (eq 6 (length name))
                 (integerp yy)
                 (integerp mm)
                 (integerp dd))
  ...
  (if (null *weblog-index-files*)
    ...
    (let* ((file (concat
      ...
      (concat "<a href=\"{blogToplevel}" file
              ...
              file "\">"
              "<img src=\"{blogToplevel}dailyLinkIcon.png\" alt=\"Daily\"></a>"))))
...
(defun weblog-macro-inside (name &optional text)
  (if (null text) (setq text name))
  (concat "<a href=\"{blogToplevel}inside/" name ".html\">" text "</A>"))
(defun weblog-macro-concept (name &optional text)
  (if (null text) (setq text name))
  (concat "<a href=\"{blogToplevel}concept/" name ".html\">" text "</A>"))
...
(defun weblog-macro-pic (align name &optional caption width)
  (let ((widtht
         (cond ((stringp width) (concat " width=\"" width "\""))
               ((numberp width) (concat " width=\"" (format "%d" width) "\""))
               (t ""))))

    (concat "<table align=\"" align "\"" widtht
          "><tr><td>"
          "<a href='javascript:openpopup(\"" *weblog-picdir* name ".jpg\", \""
          caption "\")'>"
          "<img src=\"" *weblog-picdir* name "_tn.jpg\"" widtht
          "></a></td></tr>"
          (if (null caption)
              ""
            (concat "<tr><td><p align=\"center\"><em>"
                    "<font color=\"#0000B0\">"
                    caption
                    "</font></em></p></td></tr>"))
          "</table>")))
(defun weblog-macro-picl (name &optional caption width)
  (weblog-macro-pic "left" name caption width))
(defun weblog-macro-picr (name &optional caption width)
  (weblog-macro-pic "right" name caption width))
...
(defun weblog-macro-pre ()
  "<PRE><BLOCKQUOTE><FONT COLOR=\"#000080\">")
(defun weblog-macro-prered ()
  "<PRE><BLOCKQUOTE><FONT COLOR=\"#800000\">")
(defun weblog-macro-erp ()
  "</FONT></BLOCKQUOTE></PRE>")
(defun weblog-macro-tt (text)
  (concat "<TT><FONT COLOR=\"#000080\">" text "</FONT></TT>"))
...
(defun weblog-macro-section (name)
  ...
  (concat
   ...
   "<table width=\"99%\">"
          "<tr><td bgcolor=\"black\"><font size=\"+1\" color=\"#f8f0ff\">"
          "<b>" name "</b></font></tr></td></table></a>"))

;; cvs macro defines a link to a CVS file
(defun weblog-macro-cvs (name &optional caption dir)
  (if (null dir)
      (setq name (concat "native/" name))
    (setq name (concat dir "/" name)))
  (if (null caption)
      (setq caption name))
  (concat "<A href=\"http://xlr.cvs.sourceforge.net/xlr/xl2/"
          name
          "?view=markup\">"
          caption
          "</A>"))
(defun weblog-macro-gitrev (version &optional caption)
  ...
  (setq rev
   (concat "<A href=\"http://xlr.git.sourceforge.net/git/gitweb.cgi?p=xlr/xlr;a=commit;h="
           version
           "\">"
           caption
           "</A>"))
...
(defun weblog-macro-git (name &optional caption dir)
  ...
  (weblog-macro-tt
   (concat "<A href=\"http://xlr.git.sourceforge.net/git/gitweb.cgi?p=xlr/xlr;a=blob;f="
           ...
           caption
           "</A>")))
(defun weblog-macro-svn (name &optional caption dir)
  (if (null dir)
      (setq name (concat "native/" name))
    (setq name (concat dir "/" name)))
  (if (null caption)
      (setq caption name))
  (concat "<A href=\"http://xlr.svn.sourceforge.net/viewvc/xlr/trunk/xl2/"
          name
          "?view=markup\">"
          caption
          "</A>"))
(defun weblog-macro-svnrev (rev &optional caption)
  (if (null caption)
      (setq caption (concat "Revision " (format "%d" rev))))
  (concat "<A href=\"http://xlr.svn.sourceforge.net/viewcvs.cgi/xlr?rev="
          (format "%d" rev)
          "&view=rev\">"
          caption
          "</A>"))
(defun weblog-macro-mcvs (name &optional caption dir)
  (if (null dir)
      (setq name (concat "xl/TESTS/" name))
    (setq name (concat dir "/" name)))
  (if (null caption)
      (setq caption name))
  (concat "<A href=\"http://mozart-dev.cvs.sourceforge.net/mozart-dev/mozart/"
          name
          "?view=markup\">"
          caption
          "</A>"))
...
(defvar *weblog-instable-prefix* "")
(defun weblog-macro-instable ()
...
(defun weblog-macro-inscode (head)
  (let ((result (concat
    ...
    (setq *weblog-instable-prefix* "</TD></TR>")
    result))
(defun weblog-macro-endtable ()
  (concat *weblog-instable-prefix* "</TABLE>"))


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; Implementing "Up", "Next" and "Previous" macros
;;;

(defun weblog-parse-toc-file (&optional dir)
  "Parse the *weblog-toc-file* in directory DIR.
Build the *weblog-toc* list, a list of 2 element lists,
where the first one is the file name, and the second one
the heading"
  (when (null dir) (setq dir *weblog-directory*))
  (let ((file (concat dir *weblog-toc-file*)))
    (when (file-exists-p file)
      (weblog-while-visiting-file buf file
         (weblog-parse-toc-file-buffer)))))

(defun weblog-parse-toc-file-buffer ()
  "Parse the table of content file in the current buffer.
Create the *weblog-toc* list."
  (goto-char (point-min))
  (setq *weblog-toc* ())
  (loop
   (let* ((point (point))
          (line-end (or (line-end-position) (point-max))))
     (let* ((name-pos (search-forward "<A href=\"" line-end t)))
       (when name-pos
         (let* ((end-name-pos (search-forward ".html\">" line-end t)))
           (when end-name-pos
             (let* ((end-desc-pos (search-forward "</A>" line-end t)))
               (when end-desc-pos
                 (let* ((name (buffer-substring name-pos
                                                (- end-name-pos 7)))
                        (caption (buffer-substring end-name-pos
                                                   (- end-desc-pos 4))))
                   (setq *weblog-toc*
                         (append *weblog-toc*
                                 (list (list name caption)) )))))))))
     (when (eql line-end (point-max)) (return))
     (goto-char (1+ line-end))))
  *weblog-toc*)

(defun weblog-macro-up ()
  (if *weblog-toc*
      "<A href=\"top.html\">Up</A>"
    ""))

(defun weblog-macro-next ()
  (let* ((list *weblog-toc*)
    ...
    (while list
      (setq first (car list))
      (setq list (cdr list))
      (if (and list (equal (car first) base))
          (setq result (concat "<A href=\"" (car (car list)) ".html\">Next: "
                               (car (cdr (car list))) "</A>"))))
    result))

(defun weblog-macro-previous ()
  (let* ((list *weblog-toc*)
         (base (file-name-sans-extension
                (file-name-nondirectory *weblog-story-file*)))
         (previous nil)
    ...
    (while list
      (setq first (car list))
      (setq list (cdr list))
      (if (and previous (equal (car first) base))
          (setq result (concat "<A href=\"" (car previous) ".html\">Prev: "
                               (car (cdr previous)) "</A>")))
      (setq previous first))
    result))
           ...
           (let ((string (buffer-substring (- pos 8) end-pos)))
             (when (< start-pos pos)
               ...
               (let ((header (buffer-substring start-pos (- pos 8))))
                 (when (>= 1 (count (elt "\n" 0) header))
                   ;; Insert the story attribution line, if it's no
                   ;; more than one line.
                   (setq string (concat header string)))))
             (set-buffer buf)
             (insert string)
             (insert "\n")))))
        (set-buffer buf)
